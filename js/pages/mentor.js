...
<div class="stats-grid">
  ${[['My Teams',teams.length,'#2563EB'],['Students',students.length,'#10B981'],['Announcements Sent',myAnns.length,'#8B5CF6'],['Avg Stage',teams.length?(teams.reduce((a,t)=>a+t.stage,0)/teams.length).toFixed(1):'—','#F59E0B']].map(([lbl,val,col])=>`<div class="stat-card"><div class="stat-value">${val}</div><div class="stat-label">${lbl}</div></div>`).join('')}
</div>

<div class="section-title">My Teams</div>
${teams.length===0?`<div class="empty-state card"><h3>No teams assigned yet</h3><p>Contact the administrator to assign teams.</p></div>`:
...
anns.slice(0,3).map(a=>`<div class="ann-card ${a.createdByRole}-ann"><div class="ann-meta"><span class="badge ${a.createdByRole==='admin'?'badge-blue':'badge-green'}">${a.createdByRole.toUpperCase()}</span><span style="font-size:.72rem;color:var(--text-muted)">${a.createdByName}</span></div><div class="ann-title">${a.title}</div><div class="ann-body">${a.content}</div><div class="ann-date">${new Date(a.createdAt).toLocaleString('en-IN')}</div></div>`).join('')}`;
  ...
  <div style="font-size:.8rem;color:var(--text-muted);margin-bottom:10px">CEO: ${ceo?ceo.name:'Unassigned'} · Members: ${team.members.length}</div>
...
${teams.length===0?`<div class="empty-state card"><h3>No teams assigned</h3></div>`:
teams.map(t=>_fullTeamView(t, user)).join('')}`;
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:18px;flex-wrap:wrap;gap:12px">
    <div>
      ...
      <div style="display:flex;gap:7px;flex-wrap:wrap"><span class="badge badge-blue">${team.industry}</span><span class="badge ${team.stage>=4?'badge-green':'badge-blue'}">Stage ${team.stage + 1}: ${ITE.App.STAGES[team.stage]?.label}</span></div>
    </div>
...
<div class="two-col">
  ...
  <div>
    <div class="section-title">My Teams</div>
    ${teams.map(t=>`<div class="card" style="margin-bottom:10px"><div style="font-weight:600;margin-bottom:4px">${t.startupName}</div><div style="font-size:.8rem;color:var(--text-muted)">Stage ${t.stage+1}: ${ITE.App.STAGES[t.stage]?.label} · ${t.members.length} members</div><div class="mini-progress" style="margin-top:8px">${ITE.App.STAGES.map((_,i)=>`<div class="mini-step ${i<t.stage?'done':i===t.stage?'active':''}"></div>`).join('')}</div></div>`).join('')}
  </div>
</div>`;
  }
