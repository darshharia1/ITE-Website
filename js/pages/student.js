...
ITE.Pages.Student = (function () {
  ...
  function renderDashboard() {
    ...
    const team = user.teamId ? ITE.Data.getTeamById(user.teamId) : null;
    const mentor = user.mentorId ? ITE.Data.getUserById(user.mentorId) : null;
    const tasks = ITE.Data.getTasks();
    const anns = ITE.Data.getAnnouncementsForUser(user);
    const pendingInvites = ITE.Data.getPendingInvites(user.id);
    const mySubmissions = ITE.Data.getSubmissions().filter(s => s.studentId === user.id);
...
${user.isCEO&&team&&!team.startupName?`<div class="card mt-6" style="border-color:var(--accent)"><div class="card-header"><div class="card-title">Startup Profile Setup Required</div></div><p style="font-size:.875rem;color:var(--text-secondary);margin-bottom:14px">As CEO, you are required to define the startup profile and invite team members.</p><button class="btn btn-primary" onclick="ITE.Pages.Student.showCreateStartup()">Create Startup Profile</button></div>`:''}`;
  ...
  function renderMyTeam() {
    ...
    const team = user.teamId ? ITE.Data.getTeamById(user.teamId) : null;
    const mentor = user.mentorId ? ITE.Data.getUserById(user.mentorId) : null;
    const anns = ITE.Data.getAnnouncementsForUser(user).filter(a => user.teamId && a.recipients==='team-'+user.teamId);
...
<div class="two-col">
  ...
  function _pendingInvitesSent(userId) {
    const sent=ITE.Data.getInvitations().filter(i=>i.fromUserId===userId&&i.status==='pending');
    if(!sent.length) return '';
    return `<div style="margin-top:12px"><div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:var(--text-muted);margin-bottom:6px">Pending Invitations (${sent.length})</div>${sent.map(inv=>{const to=ITE.Data.getUserById(inv.toUserId);return`<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border-subtle)"><span style="font-size:.8rem">${to?.name||'?'} → ${inv.role}</span><span class="badge badge-yellow">Pending</span></div>`;}).join('')}</div>`;
  }
  ...
  function showEditStartup(teamId) {
...
<div class="modal-footer"><button class="btn btn-ghost" onclick="ITE.App.closeModal()">Cancel</button><button class="btn btn-primary" onclick="ITE.Pages.Student._submitStartup('${teamId}')">Save Profile</button></div>
  ...
  function _submitStartup(teamId) {
    ...
    ITE.Data.updateTeam(teamId,{startupName:name,industry:ind,problemStatement:prob,description:desc});
    ITE.App.toast('Startup profile saved!','success');
    ITE.App.closeModal(); renderMyTeam();
  }

  /* ---- CEO: Invite Members ---- */
  function showInviteMember() {
    const user = ITE.Auth.getCurrentUser();
    const team = ITE.Data.getTeamById(user.teamId);
    ...
    const allStudents = ITE.Data.getStudents().filter(s=>s.id!==user.id && !team.members.some(m=>m.userId===s.id));
    ITE.App.showModal(`<div class="modal">
...
<div class="modal-body">
  ...
  <div class="form-group"><label class="form-label">Select Student</label><select id="inv-stu" class="form-control"><option value="">Choose student…</option>${allStudents.map(s=>`<option value="${s.id}">${s.name} (${s.rollNo}) · ${s.branch}</option>`).join('')}</select><div class="form-hint">You can invite any registered ITE student.</div></div>
...
<div class="modal-footer"><button class="btn btn-ghost" onclick="ITE.App.closeModal()">Cancel</button><button class="btn btn-primary" onclick="ITE.Pages.Student._submitInvite()">Send Invitation</button></div>
  ...
  function _submitInvite() {
    ...
    const user=ITE.Auth.getCurrentUser();
    // Check no existing pending invite for this person + team
    const existing=ITE.Data.getInvitations().find(i=>i.toUserId===stuId&&i.teamId===user.teamId&&i.status==='pending');
    if(existing){ITE.App.toast('A pending invitation already exists for this student.','warning');return;}
    ITE.Data.createInvitation({fromUserId:user.id,toUserId:stuId,teamId:user.teamId,role});
    ITE.App.toast(`Invitation sent to ${ITE.Data.getUserById(stuId)?.name}!`,'success');
    ITE.App.closeModal(); renderMyTeam();
  }

  function _respondInvite(invId, status) {
    const user = ITE.Auth.getCurrentUser();
    const inv = ITE.Data.getInvitations().find(i=>i.id===invId);
    if (!inv) { ITE.App.toast('Invitation not found.','error'); return; }
    ITE.Data.updateInvitation(invId, {status});
    if (status === 'accepted') {
      // Add student to team
      const team = ITE.Data.getTeamById(inv.teamId);
      if (team) {
        const members=[...team.members.filter(m=>m.userId!==user.id),{userId:user.id,role:inv.role}];
        ITE.Data.updateTeam(inv.teamId,{members});
  ...
  function renderTasks() {
    const user = ITE.Auth.getCurrentUser();
    const tasks = ITE.Data.getTasks();
    const subs = ITE.Data.getSubmissions().filter(s=>s.studentId===user.id);
